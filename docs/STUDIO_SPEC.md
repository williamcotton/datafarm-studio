...
The current implementation is version 0.1.0. It is a Vite/React application with
...
Version 0.1.0 ships two curated workflows:
...
Version 0.1.0 does not expose a general workspace model in the UI.
...
In version 0.1.0, stories are declared as `StoryBundle` values in
`src/storyBundles.ts`.
...
Version 0.1.0 ships two stories.
...
These concepts are deferred in version 0.1.0.
...
TypeScript imports. That is acceptable for version 0.1.0 but SHOULD NOT be the
...
Version 0.1.0 approximates this graph with story-level and per-section workflow
...
Version 0.1.0 runs small bundled stories and does not define strict performance
budgets.
...
Version 0.1.0 establishes the case-study alpha:
